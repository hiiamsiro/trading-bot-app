...
function makeService(overrides?: any) {
  const strategyService = {
    validateConfig: mockAsyncFn(
      (strategy: any, params: any) =>
    ...
    getRequiredCandles: mockAsyncFn(
      (strategy: any, params: any) =>
        overrides?.strategyService?.getRequiredCandles?.(strategy, params) ?? {
          entry: 16,
          trend: 16,
        },
    ),
    evaluate: mockAsyncFn(
      (input: any) =>
        overrides?.strategyService?.evaluate?.(input) ?? {
          signal: 'HOLD',
          reason: 'No signal',
          metadata: { strategy: input?.strategyKey },
        },
    ),
  };
  const marketDataAdapter = {
    getKlines: mockAsyncFn(
      (symbol: any, interval: any, limit: any) =>
        overrides?.marketDataAdapter?.getKlines?.(symbol, interval, limit) ??
        defaultCandles(symbol, interval, limit),
    ),
  };
...
test('preview throws when only 2 candles returned', async () => {
  const svc = makeService({
    marketDataAdapter: { getKlines: async () => [{ close: 100 }, { close: 101 }] },
  });
  await assertRejects(
    svc.preview({
      symbol: 'BTCUSDT',
...
test('simulate computes finalBalance = initialBalance + totalPnl', async () => {
  ...
  assert.equal(
    ...
    result.metrics.initialBalance + result.metrics.totalPnl,
  );
});
...
test('simulate uses quantity from params, falls back to 0.01', async () => {
  const svc = makeService({
    strategyService: {
      validateConfig: async (s: any, p: any) => ({
        normalizedStrategy: s,
        normalizedParams: { ...p, period: 14, oversold: 30, overbought: 70 },
      }),
      getRequiredCandles: async () => ({ entry: 16, trend: 16 }),
      evaluate: async () => ({ signal: 'HOLD', reason: 'No signal', metadata: {} }),
    },
  ...
  const result = await svc.preview({
    ...
    interval: '1m',
    strategyKey: 'rsi',
    strategyParams: { period: 14, quantity: 0 },
    sourceSymbol: 'BTCUSDT',
  });
  assert.ok(result.equityCurve.length > 0);
});
